---
title: "Reference"
output: html_vignette
vignette: >
  %\VignetteIndexEntry{Quick Search}
  %\VignetteEncoding{UTF-8}
  %\VignetteEngine{knitr::rmarkdown}
editor_options: 
  chunk_output_type: console
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
  message = FALSE,
  warning = FALSE,
  collapse = TRUE,
  comment = "#>"
)
```

```{r functions}
library(dplyr)
library(rlang)
library(fgeo)
```

```{r prefill-arguments}
# Partially fill arguments with specific details for forestgeo.
relevant_columns <- function() c("alias", "topic", "title")
fgeo_url <- "https://forestgeo.github.io/"

pick_fgeo_doc <- function(.f) {
  function(x) {
    .f(x, cols = relevant_columns(), url = fgeo_url) %>% 
      group_by(.data$title) %>% 
      mutate(
        alias = paste(unique(fgeo:::strip_s3class(.data$alias)), collapse = ", ")
      ) %>% 
      ungroup() %>% 
      select(topic, alias, title) %>% 
      unique()
  }
}
pick_fgeo_concept <- pick_fgeo_doc(fgeo:::pick_concept)
pick_fgeo_package <- pick_fgeo_doc(fgeo:::pick_package)
```

```{r, warning=FALSE}
custom_table <- function(x) {
  knitr::kable(
    x, 
    format = "html",
    col.names = NULL,
    escape = FALSE
    ) %>%
    kableExtra::column_spec(1, bold = TRUE) %>% 
    kableExtra::column_spec(2, width = "20em")
}
```

Search globally in the search box below, or navigate the index clicking the menu on the right.

### Global search

```{r, warning=FALSE}
fgeo:::fgeo_packages() %>% 
  purrr::map_dfr(pick_fgeo_package) %>% 
  unique() %>% 
  purrr::set_names(c("", "object", "description")) %>% 
  DT::datatable(
    escape = FALSE,
    rownames = NULL,
    options = list(
      # f: filter, t: table, i: information
      dom = "fti",
      search = list(search = "topography")
    )
  )
```

### Explore fgeo

```{r}
pick_fgeo_concept("functions to explore fgeo") %>% 
  custom_table()
```

### Plot

```{r}
pick_fgeo_concept("plot functions") %>% 
  custom_table()
```

### Abundance

```{r}
pick_fgeo_concept("functions for abundance and basal area") %>% 
  custom_table()
```

### Demography

```{r}
pick_fgeo_concept("demography functions") %>% 
  custom_table()
```

### Habitat

```{r}
pick_fgeo_concept("habitat functions") %>% 
  custom_table()
```

### Miscellaneous tools

#### Import ForestGEO data

```{r}
pick_fgeo_concept("functions to import ForestGEO data") %>% 
  custom_table()
```

#### Import multiple files from a directory into a list

Notice that function names have the format `input_output`.

```{r}
pick_fgeo_concept("general functions to import data") %>% 
  custom_table()
```

#### Export a list as multiple files in a directory

Notice that function names have the format `input_output`.

```{r}
pick_fgeo_concept("general functions to export data") %>%
  custom_table()
```

#### Edit data in place

```{r}
pick_fgeo_concept(c(
  "functions to edit ForestGEO data in place",
  "general functions to edit data in place"
)) %>% 
  custom_table()
```

#### Pick/drop rows with matching conditions 

```{r}
pick_fgeo_concept(c(
  "functions to pick or drop rows of a ForestGEO dataframe",
  "general functions to pick or drop rows of a dataframe"
)) %>% 
  custom_table()
```

#### Add new columns to a dataframe

```{r}
pick_fgeo_concept("functions to add columns to dataframes") %>% 
  custom_table()
```

#### Create different classes of ForestGEO data

```{r}
pick_fgeo_concept("functions to construct fgeo classes") %>% 
  custom_table()
```

### Datasets

```{r}
pick_fgeo_concept("datasets") %>% 
  custom_table()
```
