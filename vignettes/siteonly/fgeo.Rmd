...
recruitment_impl(census5, census6)
```
...
```{r}
to_df(recruitment_impl(census5, census6, split1 = census5$sp))
```
...
by_many <- recruitment_impl(census5, census6, split1 = sp_quadrat)
to_df(by_many)
```
...
```{r}
to_df(mortality_impl(census5, census6, split1 = sp_quadrat))

to_df(growth_impl(census5, census6, split1 = sp_quadrat))
```
