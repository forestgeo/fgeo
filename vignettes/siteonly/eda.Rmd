...
The data we will use is a public dataset of trees censused in a forest plot in Luquillo, Puerto Rico. That data is available in the dataset `luquillo_stem_random` from the data-package __fgeo.data__ (https://forestgeo.github.io/fgeo.data/).
...
I do not recommend this option because since one measurement is invalid, this does not mean that all of the measurements are invalid. Additionally, if you have low quality data, by the time you have applied this approach to every variable you might find that you are left with no data!
